...
Species table with number of isolates and ECVs and cutoffs where available. 
(maybe also mention source of ecvs and cutoffs?)

```{r}
species_for_tables %>%
  left_join(cutoffs_formatted) %>%
  DT::datatable(colnames =  c("Number of isolates" = "n",
                "CLSI Cutoffs" = "clsi", 
                "ECV" = "ecv"), 
                escape = FALSE)
```
...
ojs_define(specs = species_for_tables %>% select(Species))
```
...
ojs_define(data = vivli_select %>% mutate(resistance_status = ifelse(is.na(resistance_status), "No cutoffs", resistance_status), 
                                          ecv_status = ifelse(is.na(ecv_status), "No ECV", ecv_status)))
```
